...
class ResolveLanguageTest(unittest.TestCase):
    def test_names_codes_and_native_aliases(self) -> None:
        ...
        self.assertEqual(he.code, "he")
        self.assertEqual(multilingual.resolve("he").code, "he")
        self.assertEqual(multilingual.resolve("iw").code, "he")
        self.assertEqual(multilingual.resolve("עברית").code, "he")
        self.assertEqual(multilingual.resolve("Spanish").code, "es")
        self.assertEqual(multilingual.resolve("ja").code, "ja")
        self.assertEqual(multilingual.resolve("en").code, "en")

    def test_filler_words_are_stripped(self) -> None:
        self.assertEqual(multilingual.resolve("in hebrew").code, "he")
        self.assertEqual(multilingual.resolve("speak spanish").code, "es")
        self.assertEqual(multilingual.resolve("switch to japanese").code, "ja")

    def test_custom_short_names_are_accepted(self) -> None:
        klingon = multilingual.coerce("klingon")
        self.assertIsNotNone(klingon)
        self.assertEqual(klingon.name, "Klingon")
...
class LanguagePreferenceTest(IsolatedDatabaseTest):
    def test_personal_setting_injects_into_system_prompt(self) -> None:
        ...
        prompt = brain.build_system(
            "42", "tester", "hi", scope, server_name="lab"
        )
        self.assertIn("REPLY LANGUAGE", prompt)
        self.assertIn("Hebrew", prompt)
        other = brain.build_system(
            "99", "someone", "hi", scope, server_name="lab"
        )
    ...
    def test_reset_falls_back_to_guild_then_english(self) -> None:
        ...
        self.assertEqual(multilingual.effective_language("5", scope).code, "fr")
        multilingual.set_guild_language(scope, None)
        self.assertTrue(
            multilingual.is_english(multilingual.effective_language("5", scope))
        )
