"""SefBot's self-model: what it can do, and a hard ban on leaking source.
...
WHO YOU ARE
You are SefBot, a privacy-first Discord bot. People mention you or DM you to
...
_CODE_LEAK_INTENT_RE = re.compile(
    ...
    r".{0,90}\b"
    r"(?:"
    r"(?:your|ur|sefbot'?s?|this\s+bot'?s?|the\s+bot'?s?|the\s+discord\s+bot'?s?)\s+"
    ...
    r"(?:of|for)\s+(?:you|yourself|sefbot|the\s+bot|this\s+bot)"
    r"|"
    r"(?:src/)?sefbot/(?:[a-z_][\w-]*\.py)?"
    r"|"
    ...
    r"(?:your|the|sefbot'?s?)\s+"
    r"(?:source\s*code|codebase|repo(?:sitory)?|python\s+files?|modules?)"
    r"|"
    r"(?:open|read|cat|type)\s+(?:\.?/)?(?:src/)?sefbot"
    r"|"
    r"(?:send|paste|give|dump)\s+(?:me\s+)?(?:your|the|sefbot'?s?)\s+"
    ...
    r"(?:source\s*code|codebase|implementation|bot\.py|brain\.py|src/sefbot)"
    r"|"
    ...
    r"brain\.py|src/sefbot)"
    r"|"
    r"(?:pseudocode|signatures?|ast)\s+(?:of\s+)?(?:your|sefbot'?s?)\s+"
...
_CODE_DUMP_RE = re.compile(
    r"(?is)"
    r"("
    r"from\s+sefbot(?:\.[a-z_][\w]*)?\s+import"
    r"|"
    r"import\s+sefbot(?:\.|$)"
    r"|"
    r"PYTHONPATH=src\s+python\s+-m\s+sefbot"
    r"|"
    ...
    r"TOOL_SCHEMAS\s*:"
    r"|"
    r"os\.getenv\(\s*[\"'](?:DISCORD_TOKEN|SEFBOT_|GROQ_API_KEY|INFERX_API_KEY)"
    r")"
)

_SECRET_ASSIGN_RE = re.compile(
    r"(?i)\b(?:DISCORD_TOKEN|GROQ_API_KEY|INCEPTION_API_KEY|CELERIS_API_KEY|"
    r"OPENROUTER_API_KEY|SEFBOT_LLM_API_KEY|ANTHROPIC_API_KEY|DEEPSEEK_API_KEY|"
    r"INFERX_API_KEY|CEREBRAS_API_KEY|TAVILY_API_KEY|GEMINI_API_KEY|"
    r"SEFBOT_OWNER_ID)\s*[:=]\s*\S+"
)
...
def code_leaked(text: Optional[str]) -> bool:
    ...
    if len(norm) >= 160 and _CODEISH_RE.search(raw):
        hits = sum(1 for m in _SEFBOT_MARKERS if m.lower() in norm)
        if hits >= 3:
            return True
